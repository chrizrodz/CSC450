...
class Link:
	...
	def get_move(self):
		...
		if self.other_key.verify(sum(pos)+2,(int(data.split(",")[-1]),"")):
			print(pos)
	...
	def send_move(self,old_pos,new_pos):
		com = self.connection if user_player == "host" else self.s
		check_sum = old_pos[0]+old_pos[1]+new_pos[0]+new_pos[1]+2
		sign = self.my_key.sign(check_sum,"")
		message = str(self.other_key.encrypt(old_pos[0]+2,"")[0]) + "," + str(self.other_key.encrypt(old_pos[1]+2,"")[0]) + "," + str(self.other_key.encrypt(new_pos[0]+2,"")[0]) + "," + str(self.other_key.encrypt(new_pos[1]+2,"")[0]) + "," + str(sign[0])
		print(message)
		message = message.encode()
		com.sendall(message)
